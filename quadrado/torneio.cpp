...
Individuo torneio(const vector<Individuo>& populacao) {
    uniform_int_distribution<> dis_index(0, TAM_POPULACAO - 1);
    
    Individuo ind1 = populacao[dis_index(gen)];
    Individuo ind2 = populacao[dis_index(gen)];
    Individuo ind = (ind1.fitness > ind2.fitness) ? ind1 : ind2;
    return ind;
}


void crossover(const Individuo& pai1, const Individuo& pai2, Individuo& filho1, Individuo& filho2) {
    ...
    for (int i = ponto_crossover; i < TAM_CROMOSSOMO; i++) {
        swap(filho1.genes[i], filho2.genes[i]);
    }
}
...
int main(void) {
    ...
    for (int i = 0; i < NUM_GERACOES; i++) {
        ...
        while (nova_populacao.size() < TAM_POPULACAO) { 
            ...
            if (nova_populacao.size() < TAM_POPULACAO) 
